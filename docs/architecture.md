...
**Scheduling** (Optimus fork): scheduled-task management stays on the
scheduling MCP tools (`schedule_task` / `list_tasks` / `update_task` /
`cancel_task` / `pause_task` / `resume_task`), which emit `kind: 'system'`
delivery actions the host applies. Task series live in the host-only
agent-group `schedule.db` (S405); upstream's `ncl tasks` control plane is
not adopted.

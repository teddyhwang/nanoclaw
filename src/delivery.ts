...
// Lazy because module barrels register actions while delivery.ts is still
// evaluating through circular imports. A hoisted var avoids a TDZ.
// eslint-disable-next-line no-var
var deliveryActionsSingleton: Map<string, DeliveryEntry> | undefined;
function getDeliveryActions(): Map<string, DeliveryEntry> {
  if (!deliveryActionsSingleton) deliveryActionsSingleton = new Map<string, DeliveryEntry>();
  return deliveryActionsSingleton;
}
...
export function registerDeliveryAction(
  ...
  const existing = getDeliveryActions().get(action);
  if (existing) {
  ...
  getDeliveryActions().set(action, { guard: guardDecl, handler } as DeliveryEntry);
}
...
export function reenterGuardedDeliveryAction(action: string) {
  return async (ctx: { session: Session; payload: Record<string, unknown>; approval: PendingApproval }) => {
    const entry = getDeliveryActions().get(action);

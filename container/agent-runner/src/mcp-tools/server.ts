 ...
 *
 * Destinations are written by the host before every container wake. Any
 * module using this option must register after the mailbox starts (the MCP
 * bootstrap deliberately loads tool plugins in that window), because non-
 * SQLite drivers may not support lookups before `start()`. The host re-writes
 * destinations on wiring changes; agents pick up the delta next session.
 */
export interface RegisterToolsOptions {
  requireChannelType?: string[];
}

export function registerTools(tools: McpToolDefinition[], opts?: RegisterToolsOptions): void {
  if (opts?.requireChannelType && opts.requireChannelType.length > 0) {
    const reachable = opts.requireChannelType.some((ct) => hasChannelDestination(ct));
    if (!reachable) {
      const skipped = tools.map((t) => t.tool.name).join(', ');
      log(`Skipping channel-gated tools (no ${opts.requireChannelType.join('/')} destination): ${skipped}`);
      return;
    }
  }

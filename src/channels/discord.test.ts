...
describe('collapseRedundantMarkdownLinks', () => {
  ...
  it('collapses with whitespace inside the label', () => {
    const out = collapseRedundantMarkdownLinks('[ https://example.com ](https://example.com)');
  ...
  it('leaves [different label](url) alone — that markdown carries info', () => {
    const out = collapseRedundantMarkdownLinks('Open the [status page](https://status.claude.com).');
    expect(out).toBe('Open the [status page](https://status.claude.com).');
  });

  it('collapses multiple occurrences in a single message', () => {
    const out = collapseRedundantMarkdownLinks('a [https://a.com](https://a.com) b [https://b.com](https://b.com)');
    expect(out).toBe('a https://a.com b https://b.com');
  });

  it('is a no-op on text without markdown links', () => {
    const out = collapseRedundantMarkdownLinks('Plain text with a bare https://x.com URL.');
    expect(out).toBe('Plain text with a bare https://x.com URL.');
  });

  it('is idempotent', () => {
    const input = 'See [https://example.com](https://example.com) and [docs](https://example.com/docs).';
  ...
  it('does not collapse if href has trailing punctuation that label lacks', () => {
    ...
    const out = collapseRedundantMarkdownLinks('[https://x.com](https://x.com/)');

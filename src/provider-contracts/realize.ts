...
export async function realizeProviderSpawnSurfaces(
  ...
  actions: {
    ...
    skillTargets?: ReadonlyMap<string, string>;
  },
  ...
  for (const backing of contract.skillBackings) {
    ...
    syncSharedSkillLinks(skillsPath, selectedSkills, backing.conflictDiagnostics === 'warn', actions.skillTargets);
...
export function syncSharedSkillLinks(
  ...
  for (const skill of desiredSkills) {
    ...
    const target = targets?.get(skill) ?? `/app/skills/${skill}`;
    if (entry?.isSymbolicLink() && fs.readlinkSync(linkPath) !== target) {

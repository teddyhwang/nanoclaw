...
const INBOUND_KINDS = ['chat', 'chat-sdk', 'reaction', 'task', 'webhook', 'system'] as const;
...
/** One routed occurrence copied from the host-only schedule into a mailbox. */
export interface TaskOccurrenceWrite {
  id: string;
  seriesId: string;
  kind: InboundKind;
  content: string;
...
/** Container-visible, read-only projection of a live host schedule series. */
export interface TaskSeriesSnapshotRecord {
  seriesId: string;
  status: 'pending' | 'paused';
  recurrence: string | null;
  processAfter: string | null;
...
export function parseTaskOccurrenceWrite(value: unknown): TaskOccurrenceWrite {
  const record = strictRecord(value, 'TaskOccurrenceWrite', [
    'id',
    'seriesId',
    'kind',
    'content',
...
export function parseTaskSeriesSnapshotRecord(value: unknown): TaskSeriesSnapshotRecord {
  const record = strictRecord(value, 'TaskSeriesSnapshotRecord', [
    'seriesId',
    'status',
    'recurrence',
    'processAfter',
...
export function createTaskOccurrenceInboundRecord(
  occurrence: TaskOccurrenceWrite,
  sequence: number,
  timestamp: string,
): InboundRecord {
  const input = parseTaskOccurrenceWrite(occurrence);
  return parseInboundRecord({
    id: input.id,
    sequence,
    kind: input.kind,

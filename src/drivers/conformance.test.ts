...
describe('conformance: nested embedded group admission', () => {
  it('admits the exact workspace/group root and denies a sibling workspace path', () => {
    const spec = fixtureSpec();
    spec.groupHostPath = '/install/groups/workspace-a/agent-one';
    spec.containers[0].mounts.push({
      class: 'group-state',
      hostPath: '/install/groups/workspace-a/agent-one/memory',
    ...
    spec.containers[0].mounts.push({
      class: 'group-state',
      hostPath: '/install/groups/workspace-b/agent-one/memory',
      containerPath: '/workspace/sibling',
      mode: 'ro',
      groupScope: 'g1',
    });
    expect(() => validateSpec(spec, FIXTURE_POLICY)).toThrow(/denied-by-policy/);

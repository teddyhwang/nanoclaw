 ...
 * answer the question.
 */
export async function findMostRecentClosedSessionForAgent(
  agentGroupId: string,
  messagingGroupId: string,
  threadId: string | null,
): Promise<Session | undefined> {
...
export async function deleteSession(id: string): Promise<void> {
  // The grant FK has no ON DELETE CASCADE. Delete grants first so a session
  // id reused later cannot inherit another actor's sensitive-action grant.
  const db = getDb();
  const hasGrants = await hasTable(db, 'confirmation_grants');
  await db.transaction(async () => {
    if (hasGrants) await db.run('DELETE FROM confirmation_grants WHERE session_id = ?', id);
...
export async function upsertConfirmationGrant(sessionId: string, actorId: string, now: string): Promise<void> {
  const db = getDb();
  if (!(await hasTable(db, 'confirmation_grants'))) return;
  await db.run(
    ...
    { session_id: sessionId, actor_id: actorId, now },
  );
}

/** Fetch a grant, or undefined if none / table absent. */
export async function getConfirmationGrant(sessionId: string, actorId: string): Promise<ConfirmationGrant | undefined> {
  const db = getDb();
  if (!(await hasTable(db, 'confirmation_grants'))) return undefined;
  return db.get<ConfirmationGrant>(
    'SELECT * FROM confirmation_grants WHERE session_id = ? AND actor_id = ?',
    sessionId,
    actorId,
  );
}
...
export async function touchConfirmationGrant(sessionId: string, actorId: string, now: string): Promise<void> {
  const db = getDb();
  if (!(await hasTable(db, 'confirmation_grants'))) return;
  await db.run(
    'UPDATE confirmation_grants SET last_used_at = ? WHERE session_id = ? AND actor_id = ?',
    now,
    sessionId,
    actorId,
  );
...
export async function deleteConfirmationGrantsForSession(sessionId: string): Promise<void> {
  const db = getDb();
  if (!(await hasTable(db, 'confirmation_grants'))) return;
  await db.run('DELETE FROM confirmation_grants WHERE session_id = ?', sessionId);
}

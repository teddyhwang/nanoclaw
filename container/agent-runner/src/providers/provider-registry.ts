...
function providerKey(name: string): string {
  ...
  // Persisted Optimus plugin ID predates kebab-case contracts. Do not strand
  // existing container configs; new provider IDs still use the upstream grammar.
  if (name !== 'pi_rpc' && (name !== key || !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(name))) {

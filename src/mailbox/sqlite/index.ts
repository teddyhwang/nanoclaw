...
import {
  ...
  getLatestTriggeringChatRouting,
  getLatestTriggeringInbound,
  hasUserConversationMessages,
  getMessageForRetry,
  getMostRecentPeerSourceSessionId,
  getProcessingClaims,
  getRecentPendingTriggeringChats,
...
import {
  ...
  hasLiveTaskOccurrence,
  insertTaskRow,
  materializeTaskOccurrence,
  pauseTask,
  replaceTaskSeriesSnapshot,
  resumeTask,
  trailingFailedRuns,
  updateTask,
} from './tasks.js';
...
export function wrapSqliteInbound(db: Database.Database, nextSequence = () => nextEvenAcross(db)): InboundMailbox {
  return {
    ...
    getDueTasks: () => getDueTaskRows(db).map((row) => ({ id: row.id, seriesId: row.series_id, content: row.content })),
    getRecentPendingTriggeringChats: (since) =>
      getRecentPendingTriggeringChats(db, since).map((row) =>
        parseInboundWrite({
          id: row.id,
          kind: row.kind,
          timestamp: sqliteTimestamp(row.timestamp),
    ...
    markMessagesCompleted: (messageIds) => markMessagesCompleted(db, messageIds),
    getLatestTriggeringChatRouting: () => {
      const row = getLatestTriggeringChatRouting(db);
      return row && { channelType: row.channel_type, platformId: row.platform_id };
    },
    getLatestTriggeringInbound: () => getLatestTriggeringInbound(db),
    hasUserConversationMessages: (messageIds) => hasUserConversationMessages(db, messageIds),
    getLatestHumanInboundMs: () => getLatestHumanInboundMs(db),
    getRecoverableMessage: (messageId) => {
      const row = getRecoverableMessage(db, messageId);
      return (
        row && {
          ...row,
    ...
    getFailedMessageNotifyInfo: (messageId) => {
      const row = getFailedMessageNotifyInfo(db, messageId);
      return (
        row && {
          kind: row.kind,
          trigger: row.trigger === 1,
    ...
    gcStaleSystemRows: (now) => gcStaleSystemRows(db, now),
    wasDeliveredByBot: (platformMessageId) => wasDeliveredByBot(db, platformMessageId),

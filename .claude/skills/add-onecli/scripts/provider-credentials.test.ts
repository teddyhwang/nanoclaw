...
const metadata = (spec: OneCliCredential = google) => ({
  id: 'existing-key',
  name: spec.name,
  type: spec.type,
...
describe('OpenCode vault management', () => {
  ...
  it('reads the gateway saved after the setup process imported configuration', async () => {
    ...
    try {
      ...
      await createOneCliCredentialConnection(google, undefined, undefined, transport).find();
    ...
    const transport = vi.fn(
      async (_url: string, init: RequestInit) =>
        new Response(
          JSON.stringify(
            init.method === 'GET'
              ? [{ ...metadata(), injectionConfig: { headerName: 'Authorization', valueFormat: 'Bearer {value}' } }]
              : { success: true },
          ),
        ),
    );
    await createOneCliCredentialConnection(google, 'http://vault.example', '', transport as typeof fetch).keep(
      'existing-key',
    );
  ...
  it('creates a correctly mapped Google key and returns no response preview', async () => {
    ...
    const vault = createOneCliCredentialConnection(google, 'http://vault.example', '', transport as typeof fetch);
  ...
  it('repairs the recognized old Google bearer mapping without replacing the granted ID', async () => {
    const old = { ...metadata(), injectionConfig: { headerName: 'Authorization', valueFormat: 'Bearer {value}' } };
    const transport = vi.fn(
      async (_url: string, init: RequestInit) =>
  ...
  it('names all mismatched fields without exposing their values or the secret response', () => {
    ...
    expect(() => findOneCliCredential([entry], CHATGPT_SECRET)).toThrow(
      'unexpected metadata in: scope, pathPattern, metadata.authMode',
    );
    expect(() => findOneCliCredential([entry], CHATGPT_SECRET)).not.toThrow('private-');
  });

  it('refuses duplicates and a changed identity instead of creating another secret', async () => {
    expect(() => findOneCliCredential([metadata(), metadata()], google)).toThrow('Multiple');
    const transport = vi.fn(async () => new Response(JSON.stringify([{ ...metadata(), id: 'changed' }])));
    await expect(
      createOneCliCredentialConnection(google, 'http://vault.example', '', transport).save(
        'key-fixture',
        'existing-key',
      ),
  ...
  it('preserves ChatGPT OAuth metadata and rejects inherited names', () => {
    expect(findOneCliCredential([metadata(CHATGPT_SECRET)], CHATGPT_SECRET)).toBe('existing-key');
    expect(() =>
      findOneCliCredential([{ ...metadata(CHATGPT_SECRET), scope: 'organization' }], CHATGPT_SECRET),
    ).toThrow();
  });
});
...
describe('OpenCode credential host migration', () => {
  ...
  it.each(['keep', 'replace'])('requires confirmation and updates the same granted ID on %s', async (mode) => {
    ...
    const vault = createOneCliCredentialConnection(target, 'https://vault.example', '', transport as typeof fetch);
  ...
  it.each([
    ...
    const confirmHostChange = vi.fn(async () => true);
    await expect(
      createOneCliCredentialConnection(target, 'https://vault.example', '', transport).find({ confirmHostChange }),
  ...
  it.each(['identity', 'host', 'scope'])('refuses a concurrent %s change after confirmation', async (change) => {
    ...
    const vault = createOneCliCredentialConnection(target, 'https://vault.example', '', transport);
  ...
  it('keeps ChatGPT host validation strict even when confirmation is supplied', async () => {
    ...
    const confirmHostChange = vi.fn(async () => true);
    await expect(
      createOneCliCredentialConnection(CHATGPT_SECRET, 'https://vault.example', '', transport).find({
        confirmHostChange,
      }),
    ).rejects.toThrow('unexpected metadata');
    expect(confirmHostChange).not.toHaveBeenCalled();

 ...
 */
export async function setMessagingGroupPlatformId(id: string, newPlatformId: string): Promise<boolean> {
  const result = await getDb().run(
    'UPDATE messaging_groups SET platform_id = ? WHERE id = ? AND platform_id != ?',
    newPlatformId,
    id,
    newPlatformId,
  );
 ...
 */
export async function setAgentGroupSessionMode(
  agentGroupId: string,
  sessionMode: MessagingGroupAgent['session_mode'],
): Promise<number> {
  const result = await getDb().run(
    'UPDATE messaging_group_agents SET session_mode = ? WHERE agent_group_id = ? AND session_mode != ?',
    sessionMode,
    agentGroupId,
    sessionMode,
  );

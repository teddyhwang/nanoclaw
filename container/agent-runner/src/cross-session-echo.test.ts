...
describe('routing (extractRouting)', () => {
  ...
  it('keeps an all-echo batch non-addressable (defensive)', () => {
    insertEcho('e1', { seq: 2 });
    insertEcho('e2', { seq: 4 });
    ...
    expect(routing.taskFire).toBe(false);
  });

  it('taskFire stays true when echo rows ride along with a task', () => {
  ...
  it('taskFire is false when a real chat row is in the batch', () => {
    insertMessage('t1', 'task', { prompt: 'daily digest' }, { seq: 2 });
    insertMessage('m1', 'chat', { sender: 'Alice', text: 'hi' }, { seq: 4 });

    expect(extractRouting(getPendingMessages()).taskFire).toBe(false);
  });

  it('taskFire is false without any task row (echo-only batch)', () => {
    insertEcho('e1', { seq: 2 });

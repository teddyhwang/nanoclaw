...
import { addSkillRoot, _resetSkillRootsForTests } from './engine/skill-roots.js';
...
beforeEach(async () => {
  ...
  fs.mkdirSync(path.join(TEST_ROOT, 'container', 'agent-runner', 'src'), { recursive: true });
  fs.mkdirSync(path.join(TEST_ROOT, 'container', 'skills'), { recursive: true });
  setEnginePaths({
    projectRoot: TEST_ROOT,
    dataDir: DATA_DIR,
    groupsDir: GROUPS_DIR,
    containerSourceDir: path.join(TEST_ROOT, 'container'),
  });
...
describe('buildMounts agent surfaces', () => {
  ...
  it('materializes the Claude surface access contract', async () => {
    ...
    expect(byContainerPath.get('/workspace/agent/CLAUDE.md')).toMatchObject({
      hostPath: fs.realpathSync(path.join(GROUPS_DIR, ag.folder, 'CLAUDE.md')),
      readonly: true,
    });
...
it('declared Claude surfaces keep embedded skill targets and plugin mounts across refreshes', async () => {
  const ag = group('embedded-contract-skills', 'embedded-contract-skills');
  await createAgentGroup(ag);
  await ensureContainerConfig(ag.id);
  const sess = session('embedded-sess', ag.id);
  const root = path.join(TEST_ROOT, 'host-skills');
  fs.mkdirSync(path.join(root, 'browser'), { recursive: true });
  addSkillRoot({ hostPath: root, containerPath: '/plugins/host-skills' });
  const config: ContainerConfig = { ...containerConfig(), skills: 'all' };
  ...
  fs.unlinkSync(path.join(skills, 'browser'));
  fs.symlinkSync('/app/skills/browser', path.join(skills, 'browser'));

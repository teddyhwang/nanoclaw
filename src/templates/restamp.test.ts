...
/**
 * Put a series into the non-live state restamp must refuse to write through.
 * Upstream simulates this with a `completed` messages_in row awaiting the
 * sweep's re-arm; this fork's series row stays `pending` across fires, so the
 * equivalent non-live-but-present state is `cancelled`.
 */
function makeSeriesNonLive(groupId: string, seriesId: string): void {
  const db = openScheduleDb(groupId);
  try {
    db.prepare("UPDATE task_series SET status = 'cancelled' WHERE series_id = ?").run(seriesId);
  } finally {
    db.close();
  }
}

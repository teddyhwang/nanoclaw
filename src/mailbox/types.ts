...
export interface LatestTriggeringChatRouting {
  channelType: string;
  platformId: string;
}
...
export interface RecoverableMessage extends MessageRetry {
  status: 'pending' | 'processing';
}

export interface FailedMessageNotifyInfo {
  kind: string;
  trigger: boolean;
...
export interface TriggeringInboundSummary {
  kind: string;
  content: string;
}
...
export interface InboundMailbox {
  ...
  getDueTasks(): DueTask[];
  getRecentPendingTriggeringChats(since: string): InboundMessage[];
  markMessagesCompleted(messageIds: readonly string[]): void;
  getLatestTriggeringChatRouting(): LatestTriggeringChatRouting | undefined;
  getLatestTriggeringInbound(): TriggeringInboundSummary | undefined;
  hasUserConversationMessages(messageIds: readonly string[]): boolean;
  getLatestHumanInboundMs(): number;
  getRecoverableMessage(messageId: string): RecoverableMessage | undefined;
  getFailedMessageNotifyInfo(messageId: string): FailedMessageNotifyInfo | undefined;
  gcStaleSystemRows(now?: number): number;
  wasDeliveredByBot(platformMessageId: string): boolean;
  markMessageFailed(messageId: string): void;
  retryWithBackoff(messageId: string, backoffSec: number): void;
  /** @deprecated Use getRecoverableMessage for claim recovery. */
  ...
  hasLiveTaskOccurrence(seriesId: string): boolean;
  materializeTaskOccurrence(occurrence: TaskOccurrenceWrite): Promise<void>;
  replaceTaskSeriesSnapshot(series: readonly TaskSeriesSnapshotRecord[]): void;

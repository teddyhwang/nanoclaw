...
import { getPluginMigrations } from '../../engine/db-extensions.js';
...
// Optimus fork migrations share upstream's 016/017 numbering but are keyed
// (and deduped) by NAME, so the collision is import-alias-only.
import { migration016 as migration016Optimus } from './016-optimus-container-config-fields.js';
import { migration017 as migration017Optimus } from './017-optimus-sensitive-gate-mode.js';
...
export async function runMigrations(
  ...
  // Embedded-host plugin migrations follow all core/module migrations and use
  // the same locking, transaction, override, and schema-version machinery.
  const resolvedList: readonly Migration[] = list ?? [
    ...getRegisteredMigrations(),
    ...getPluginMigrations().map((migration) => ({
  ...
  if (mode === 'validate') {
    ...
    const pending = resolvedList.filter((migration) => !applied.has(migration.name));
    if (pending.length > 0) {
      throw new Error(

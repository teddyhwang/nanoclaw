...
describe('lifecycle', () => {
  ...
  it('retains a 50-line stderr crash tail', async () => {
    const handle = await driver().prepare(fixtureSpec());
    await handle.start();
    const proc = cli.started.at(-1)!.proc;
    for (let i = 0; i < 60; i++) proc.emitStderr(`line-${i}`);
    proc.emitExit(1);

    expect(log.warn).toHaveBeenCalledWith(
      'Container exited non-zero',
      expect.objectContaining({
        stderrTail: Array.from({ length: 50 }, (_, index) => `line-${index + 10}`),
      }),
    );
  });
...
describe('idempotency and adoption', () => {
  ...
  it('uses the 12-second checkpoint grace for adopted handles', async () => {
    cli.responses = [{ match: /^ps -a/, output: 'ncl-spike-s1|running|g1|s1\n' }];
    const [snapshot] = await driver().listSessions('spike');

    await snapshot.handle.stop('host-shutdown');

    expect(cli.joined()).toContain('stop -t 12 ncl-spike-s1');
  });

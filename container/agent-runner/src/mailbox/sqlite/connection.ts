...
export function withInboundDb<T>(fn: (db: Database) => T): T {
  ...
  for (let attempt = 0; attempt <= INBOUND_CORRUPT_RETRIES; attempt++) {
    ...
    try {
      // Opening is inside the retry boundary: VirtioFS torn windows can fail
      // before a handle exists (SQLITE_CANTOPEN/NOTADB), not only on reads.
      db = openInboundDb();
      return fn(db);
    } catch (err) {
      lastErr = err;
      if (!isTransientInboundError(err) || attempt === INBOUND_CORRUPT_RETRIES) {
        throw err;
      }
      // Transient torn read/open — close, let the host write settle, reopen.
      // Synchronous busy-wait (not setTimeout): callers are synchronous and
      // the backoff is far cheaper than a crash plus host respawn.
 ...
 * race (SQLITE_CANTOPEN/NOTADB/CORRUPT — see isTransientInboundError). This

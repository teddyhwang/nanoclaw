...
export function contributionFromArgs(args: readonly string[], groupScope: string): GatewayContribution {
  ...
  for (let i = 0; i < args.length; i += 2) {
    ...
    if (flag === '-e' && value?.includes('=')) {
      ...
      const rawKey = value.slice(0, eq);
      const envValue = value.slice(eq + 1);
      // The Claude SDK prefers x-api-key when ANTHROPIC_API_KEY is present,
      // defeating OneCLI's OAuth/Bearer replacement and sending the literal
      // placeholder upstream. Normalize the stub to the Bearer-token lane.
      const key = rawKey === 'ANTHROPIC_API_KEY' && envValue === 'placeholder' ? 'ANTHROPIC_AUTH_TOKEN' : rawKey;
      env[key] = envValue;
      continue;
    }
    if (flag === '-v' && value) {

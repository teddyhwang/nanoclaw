...
describe('assistant text block surfacing', () => {
  ...
  it('keeps an earlier wrapped assistant message on the text door and completes without replay on a retryable result', async () => {
    const streamed = '<message to="user">The useful answer already streamed.</message>';
    sdkMessages.length = 0;
    sdkMessages.push(
      { type: 'system', subtype: 'init', session_id: 'sess-rate-limit' },
      { type: 'assistant', message: { content: [{ type: 'text', text: streamed }] } },
      {
        type: 'result',
        subtype: 'error_during_execution',
        is_error: true,
        errors: ["API Error: Request rejected (429) · This request would exceed your account's rate limit."],
      },
    );
    ...
    const events: Array<{ type: string; text?: string | null; classification?: string }> = [];
    for await (const event of q.events) events.push(event);

    expect(events.filter((event) => event.type === 'text').map((event) => event.text)).toEqual([streamed]);
    expect(events.filter((event) => event.type === 'result').map((event) => event.text)).toEqual([null]);

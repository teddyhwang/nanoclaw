...
- [BREAKING] **Provider-agnostic memory.** All providers share one OKF v0.1-compatible `memory/` tree, while persona lives in `instructions.prepend.md`; startup, clear, and compact reload memory automatically. Existing groups with legacy memory must run `/migrate-memory` before use. See [memory](docs/memory.md) and [provider migration](docs/provider-migration.md).
- [BREAKING] **Channel install skills are now the single source of truth.** The setup wizard applies the same `/add-<channel>` skills used interactively; the bespoke non-interactive and per-channel wizard installers were removed. See [skill directives](docs/skill-directives.md).
- **The guard seam.** Privileged actions crossing container or channel boundaries now pass through `guard()` before execution; approved replays carry a durable grant and re-run structural checks.
...
- **Pre-task script failures back their series off instead of spinning.** A script error lands the occurrence as a failed run (`script-skip:error` ack → `failed` status); recurrence reads the series' trailing failed streak and re-arms at `max(cron next, now + 2·2^(n−1) min, cap 60)`; after 8 consecutive failures the series is auto-paused (`resume_task` revives it). A deliberate `wakeAgent:false` gate is a normal run and never backs off. Also fixed: an explicitly-addressed `<message to>` in a task fire's final text now delivers as a deliberate send; identical echoes of an MCP send are dropped in the runner, where the duplication originates.
- **Optimus fork divergence:** upstream moved scheduled tasks to `ncl tasks`; this fork retains the six scheduling MCP tools and host-only per-agent-group `schedule.db` storage required by S405. See [docs/ncl-tasks-migration.md](docs/ncl-tasks-migration.md).
...
- [BREAKING] **Chat SDK pinned to `4.34.0` (was `4.26.0` via `^4.24.0`).** `chat` and the `@chat-adapter/*` channel adapters are version-locked — the adapter's `ChatInstance` must match the bridge's, so a mismatched pair fails to typecheck at `createChatSdkBridge(...)`. `chat` is therefore pinned exactly, and the channel-adapter install pins move with it — the `/add-<channel>` SKILL.md steps and `setup/*.sh` scripts on `main`, plus the adapter code on the `channels` branch. Core installs with no channel (only `cli`) are unaffected. **Migration:** if any channel is installed (Slack, Discord, Telegram, Teams, …), re-run its `/add-<channel>` skill to pull the matching `4.34.0` adapter.

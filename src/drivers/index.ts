 ...
 * explicit host-gateway mapping needed to reach host services on Linux and
 * recent Docker Desktop releases. Injected at
...
export function dockerNetworkArgs(spec: SessionSpec): string[] {
  ...
  return [`--add-host=${spec.networkAccess.endpoint}:host-gateway`];
}
...
export function mountPolicy(env: NodeJS.ProcessEnv = process.env): MountPolicy {
  ...
  const containerSourceDir = path.resolve(env.NANOCLAW_CONTAINER_SOURCE_DIR || paths.containerSourceDir);
  const canonical = (candidate: string): string => (fs.existsSync(candidate) ? fs.realpathSync(candidate) : candidate);
  return {

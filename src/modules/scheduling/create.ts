...
import { openScheduleDb, pauseSeries, upsertSeries } from './schedule-store.js';
...
export function createScheduledTask(
  ...
  try {
    ...
    if (options?.status === 'paused') pauseSeries(db, id);
  } finally {
    db.close();
  }
  ...
  return {
    session: { id: `schedule:${agentGroupId}`, agent_group_id: agentGroupId },
    row: {
      row_id: id,
      series_id: id,

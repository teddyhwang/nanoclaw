...
describe('claim recovery', () => {
  it('returns and resets both pending and processing rows, never terminal rows', () => {
    const db = new Database(':memory:');
    db.exec(INBOUND_SCHEMA);
    for (const [index, status] of ['pending', 'processing', 'completed'].entries()) {
      db.prepare('INSERT INTO messages_in (id, seq, kind, timestamp, status, content) VALUES (?, ?, ?, ?, ?, ?)').run(
        `msg-${status}`,
        (index + 1) * 2,
        'chat',
    ...
    expect(getRecoverableMessage(db, 'msg-pending')?.status).toBe('pending');
    expect(getRecoverableMessage(db, 'msg-processing')?.status).toBe('processing');
    expect(getRecoverableMessage(db, 'msg-completed')).toBeUndefined();

    retryWithBackoff(db, 'msg-processing', 1);
    expect(db.prepare('SELECT status, tries FROM messages_in WHERE id = ?').get('msg-processing')).toEqual({
      status: 'pending',
      tries: 1,
    });
    retryWithBackoff(db, 'msg-completed', 1);
    expect(db.prepare('SELECT status, tries FROM messages_in WHERE id = ?').get('msg-completed')).toEqual({
      status: 'completed',
      tries: 0,
    });
    db.close();

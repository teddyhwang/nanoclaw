...
export interface SessionSpec {
  key: SessionKey;
  /**
   * Exact resolved host root for this group's writable state. Embedded hosts
   * may nest it below workspace slugs, so it cannot be reconstructed from the
   * flat group-folder label. New composers always set it; the optional shape
   * preserves admission of pre-field flat specs during upgrades.
   */
  groupHostPath?: string;
  /** Exact read-only shared-group/global state roots trusted for this session. */
...
export function validateSpec(spec: SessionSpec, policy: MountPolicy, capabilities?: DriverCapabilities): void {
  if (spec.groupHostPath) {
    if (!hostPathCanonical(spec.groupHostPath) || !underRoot(spec.groupHostPath, policy.groupsRoot)) {
      throw deniedByPolicy(`groupHostPath ${spec.groupHostPath} must be a canonical subtree of ${policy.groupsRoot}`);
    }
  }
  for (const root of spec.sharedStateRoots ?? []) {
    if (!hostPathCanonical(root) || !underRoot(root, policy.groupsRoot)) {
      throw deniedByPolicy(`shared state root ${root} must be a canonical subtree of ${policy.groupsRoot}`);
    }
  }
...
export function stampedPluginsRoot(spec: SessionSpec, policy: MountPolicy): string | null {
  const root = resolvedGroupHostPath(spec, policy);
  return root ? `${root}/plugins` : null;
}

function resolvedGroupHostPath(spec: SessionSpec, policy: MountPolicy): string | null {
  if (spec.groupHostPath) return spec.groupHostPath;
  const folder = spec.labels[GROUP_FOLDER_LABEL];
  if (!folder || !labelValueLegal(folder)) return null;
  return `${policy.groupsRoot}/${folder}`;
}
...
function mountAllowed(mount: MountSpec, spec: SessionSpec, policy: MountPolicy): boolean {
  switch (mount.class) {
    ...
    case 'group-state': {
      ...
      const groupRoot = resolvedGroupHostPath(spec, policy);
      return groupRoot !== null && underRoot(mount.hostPath, groupRoot);
    }
    default:
      return false;
  }

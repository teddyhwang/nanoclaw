...
// Keep template stamping tests isolated from host runtime registration;
// schedule-store only needs this path helper.
vi.mock('../session-manager.js', () => ({
  sessionsBaseDir: () => '/tmp/nanoclaw-restamp-test/data/v2-sessions',
}));
// Secret-classifier behavior belongs to mcp.test.ts. These tests only need the
// parser constants without loading the approvals/delivery module graph.
vi.mock('../modules/self-mod/request.js', () => ({
  SECRET_ENV_KEY_RE: /(TOKEN|SECRET|PASSW(OR)?D|API_?KEY|APIKEY|CREDENTIAL|PRIVATE_?KEY|AUTH)/i,
  SECRET_VALUE_RE: /^(sk-|ghp_|github_pat_|xox[a-z]-|AKIA|-----BEGIN )/,
}));
...
import { materializeTaskOccurrence } from '../mailbox/sqlite/tasks.js';
import { createScheduledTask, prepareScheduledTask } from '../modules/scheduling/create.js';
import { openScheduleDb, resumeSeries } from '../modules/scheduling/schedule-store.js';
...
const OCCURRENCE_DB = path.join(TEST_ROOT, 'occurrence.db');
...
// Fork divergence (S405): tasks live as one row per series in the group's
// host-only schedule.db, not as messages_in rows across task sessions.
function liveTasks(groupId: string): Array<{ series_id: string; status: string; recurrence: string; prompt: string }> {
  const db = openScheduleDb(groupId);
  try {
    return (
      db
        .prepare(
          "SELECT series_id, status, recurrence, content FROM task_series WHERE kind = 'task' AND status IN ('pending', 'paused')",
        )
        .all() as Array<{ series_id: string; status: string; recurrence: string; content: string }>
    ).map((row) => ({ ...row, prompt: (JSON.parse(row.content) as { prompt: string }).prompt }));
  } finally {
    db.close();
  }
}
...
function allTaskSeries(groupId: string): string[] {
  ...
  try {
    return (
      db.prepare("SELECT DISTINCT series_id FROM task_series WHERE kind = 'task'").all() as Array<{
        series_id: string;
      }>
...
describe('restampAgentFromTemplate', () => {
  ...
  it('applies the full update across every plugin-owned surface', async () => {
    ...
    const scheduleDb = openScheduleDb(g.id);
    try {
      resumeSeries(scheduleDb, series);
    } finally {
      scheduleDb.close();
    }
    writeTemplateV2();
  ...
  it('updates a due series without duplicating it or mutating its materialized occurrence', async () => {
    const g = await stamp();
    const series = liveTasks(g.id)[0].series_id;
    const schedule = openScheduleDb(g.id);
    let occurrenceContent: string;
    try {
      resumeSeries(schedule, series);
      schedule
        .prepare('UPDATE task_series SET process_after = ? WHERE series_id = ?')
        .run('2020-01-01T00:00:00.000Z', series);
      occurrenceContent = (
        schedule.prepare('SELECT content FROM task_series WHERE series_id = ?').get(series) as {
          content: string;
        }
      ).content;
    } finally {
    ...
    ensureSchema(OCCURRENCE_DB, 'inbound');
    const inbound = openInboundDb(OCCURRENCE_DB);
    try {
      materializeTaskOccurrence(inbound, {
    ...
    expect(change(result.changes, 'task', 'weekday-briefing').action).toBe('update');
    expect(allTaskSeries(g.id).filter((id) => id.startsWith('weekday-briefing-'))).toEqual([series]);

    const occurrenceDb = openInboundDb(OCCURRENCE_DB);
    try {
      const row = occurrenceDb.prepare('SELECT content FROM messages_in WHERE id = ?').get('task-materialized') as {
        content: string;
      };
      expect(JSON.parse(row.content).prompt).toBe('Send the weekday briefing.');
    } finally {
      occurrenceDb.close();
    }
    expect(liveTasks(g.id).find((task) => task.series_id === series)?.prompt).toBe(
      'Send the improved weekday briefing.',
    );
  });

  it('deletes a dropped template series from schedule.db', async () => {
    const g = await stamp();
    writeTemplateV2();

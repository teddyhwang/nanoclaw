...
export function restampAgentFromTemplate(ref: string, agentGroupId: string, opts: { apply: boolean }): RestampResult {
  ...
  for (const task of tpl.tasks) {
    ...
    ops.push(() => {
      const db = openScheduleDb(group.id);
      try {
        updateSeries(db, match.seriesId, {
  ...
  for (const [slug, oldTask] of oldTasks) {
    ...
    ops.push(() => {
      // Fork divergence (S405): upstream deletes the task rows from the
      // session inbound.db; here a series is one row in the group's
      // schedule.db, so the same terminal removal is a single DELETE.
      const db = openScheduleDb(group.id);
      try {
        db.prepare("DELETE FROM task_series WHERE series_id = ? AND kind = 'task'").run(match.seriesId);
      } finally {
        db.close();
      }
    });
 ...
 * keeps one `task_series` row per series in a per-agent-group `schedule.db`, so
 * the lookup is a single indexed read over that table instead.
 *
 * Upstream's oldest-session-first scan still has meaning here and is preserved
...
function findTaskSeriesBySlug(agentGroupId: string, slug: string): TaskSeriesMatch | undefined {
  ...
  const db = openScheduleDb(agentGroupId);
  try {
    const row = db
      .prepare(

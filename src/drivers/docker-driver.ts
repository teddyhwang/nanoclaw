...
const ADOPTED_STOP_GRACE_SECONDS = 12;
...
class DockerHandle implements SessionHandle {
  ...
  async start(): Promise<void> {
    ...
    const proc = this.cli.start(['start', '--attach', this.name], { captureStdout: true });
    this.#proc = proc;
    const logDir = process.env.NANOCLAW_CONTAINER_LOG_DIR;
    if (logDir) {
      try {
        fs.mkdirSync(logDir, { recursive: true });
        this.#logStream = fs.createWriteStream(`${logDir}/container-${this.name}.log`, { flags: 'a' });
        this.#logStream.write(`=== start ${new Date().toISOString()} session=${this.key.sessionId} ===\n`);
      } catch (err) {
        this.#logStream = null;
        log.debug('Container log capture disabled', { container: this.name, err });
      }
    }
    proc.onStderr((line) => {
    ...
    proc.onStdout((chunk) => this.#logStream?.write(chunk));
    proc.onExit((code) => {
      ...
      if (!this.#stopping && code !== 0 && code !== null) {
        log.warn('Container exited non-zero', {
          ...
          code,
          stderrTail: this.#stderrTail.length > 0 ? this.#stderrTail : ['(no stderr captured)'],
        });
      }
      this.#logStream?.end();
      this.#logStream = null;
  ...
  async stop(reason: string): Promise<void> {
    ...
    const grace = String(this.pendingSpec?.stopGraceSeconds ?? ADOPTED_STOP_GRACE_SECONDS);
    try {

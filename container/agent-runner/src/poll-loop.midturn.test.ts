...
describe('mid-turn <message> block delivery', () => {
  ...
  it('still nudges an unwrapped result when no mid-turn block was delivered', async () => {
    ...
    expect(getUndeliveredMessages().filter((message) => message.kind === 'chat')).toHaveLength(0);
  ...
  it('per-turn sent count resets: a later bare unwrapped turn still nudges after an earlier mid-turn send', async () => {
    ...
    expect(getUndeliveredMessages().filter((message) => message.kind === 'chat')).toHaveLength(1);
  ...
  it('task runs: a deliberate mid-turn block is delivered once and not replayed at the result door', async () => {
    seedDest();
    const block = '<message to="discord-main">digest is ready</message>';
    async function* events(): AsyncGenerator<ProviderEvent> {
      yield { type: 'init', continuation: 's1' };
      yield { type: 'text', text: block };
      yield { type: 'result', text: block };
    ...
    const chat = getUndeliveredMessages().filter((message) => message.kind === 'chat');
    expect(chat).toHaveLength(1);
    expect(JSON.parse(chat[0].content).text).toBe('digest is ready');
    expect(chat[0].in_reply_to).toBeNull();
  });
});

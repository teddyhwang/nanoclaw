...
#### Scheduling tools (Optimus fork)

Schedule, inspect, and modify one-shot or recurring tasks via the scheduling
MCP tools: `schedule_task`, `list_tasks`, `update_task`, `cancel_task`,
`pause_task`, `resume_task` (upstream's `ncl tasks` control plane is not
adopted here).

Implementation: the tools emit `kind: 'system'` delivery actions the host
applies. Task series live in the host-only agent-group `schedule.db` (S405);
the host sweep materializes one due occurrence per fire into the session's
`inbound.db` and wakes the container. Tasks may carry a pre-task script whose
last stdout line is a `{"wakeAgent": bool, ...}` JSON verdict gating the wake.
...
### Embedded Optimus compatibility (2026-09-11)

- Load host provider plugins before validating the configured provider name. The persisted `pi_rpc` ID remains a legacy exception to new kebab-case IDs.
- Resolve each provider instance's memory capability before wrapping it for quota failover. Delivery mode follows the active query/contract, not the standing provider or stale legacy flags.
- Keep reply routes queued by provider result boundary, including wrapping retries and suppressed Codex results. Optimus still picks the newest triggering chat in each batch and persists an explicit no-reply target for task-only turns.
- Task results retain host-owned `schedule.db` and `task_fires`; upstream `ncl tasks`/`task_log` delivery is intentionally not adopted. Preserve distinct outcomes after acknowledgments.
- Host reconciliation now runs through the keyed queue and durable incarnation fence; Optimus adaptive idle/MCP/startup grace, bounded sweeps, retry notices and scheduling projections live in `src/reconcile-session.ts`. Embedded boot registers the lease before adoption and starts/stops host lifecycle modules.
- Declared provider surfaces retain workspace path injection, host skill targets and spawn-plugin mounts. The frozen image runtime/SDK bump takes effect only after an explicit image rebuild; an adopted process does not reload mounted source.

...
export async function initChannelAdapters(setupFn: (adapter: ChannelAdapter) => ChannelSetup): Promise<void> {
  ...
  for (const [name, registration] of registry) {
    try {
      ...
      nameToChannelType.set(name, key);
...
export async function startChannelAdapter(key: string): Promise<'started' | 'already-active' | 'no-credentials'> {
  ...
  if (!bootSetupFn) throw new Error('startChannelAdapter: initChannelAdapters has not run');
  const adapter = await registration.factory();
  if (!adapter) return 'no-credentials';
  const setup = bootSetupFn(adapter);
  let attempt = 0;
  while (true) {
    try {
      await adapter.setup(setup);
  ...
  nameToChannelType.set(key, activeKey);

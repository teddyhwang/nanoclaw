...
import { getContinuationStartedAt, clearContinuationStartedAt } from '../db/session-state.js';
...
export function rotateClaudeContinuation(
  ...
  try {
    ...
    let reason = decideContinuationRotation({ size, firstLine }, fx)?.reason;
    if (!reason && input.readSessionStats) {
      let startedAt: string | undefined;
      try {
        startedAt = getContinuationStartedAt('claude');
      } catch {
        /* boot/test without DB */
      }
      const decision = evaluateRotation(new Date(fx.now()), TIMEZONE, startedAt, input.readSessionStats());
      if (decision.rotate) reason = decision.reason;
    }

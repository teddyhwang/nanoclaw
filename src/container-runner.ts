...
/** Codex and other providers need time to checkpoint an active turn. */
const STOP_GRACE_SECONDS = 12;
...
async function spawnContainer(session: Session): Promise<void> {
  ...
  // Resolve all spawn lanes once. Last contributor wins within the plugin
  // lane; precedence across lanes is provider < spawn plugins < credential < gateway.
  const {
    provider,
  ...
  const spawnContribution = await collectSpawnContributions({
  ...
  const contribution: ProviderContainerContribution = {
    mounts: [...(providerContribution.mounts ?? []), ...spawnContribution.mounts],
    env: { ...(providerContribution.env ?? {}), ...spawnContribution.env },
  };
  const { getCredentialProvider } = await import('./engine/credentials.js');
  const credential = (await getCredentialProvider()?.resolve({ agentGroupId: agentGroup.id })) ?? {};
...
async function finish(sessionId: string, runtime: ActiveSessionRuntime, failure?: SessionFailure): Promise<void> {
  ...
  try {
    const agentGroup = await getAgentGroup(runtime.handle.key.agentGroupId);
    if (agentGroup) cleanupSeededDream(agentGroup);
  } catch (err) {
    log.warn('Failed to resolve group during container finalization', { sessionId, err });
  }
  try {
    emitEngineEvent('container.stop', {
      sessionId,
      agentGroupId: runtime.handle.key.agentGroupId,
      reason: containerStopEventReason(runtime.stopReason, failure),
    });
  } catch (err) {
    log.error('container.stop listener threw', { sessionId, err });
  }
...
function containerStopEventReason(
  stopReason: string | undefined,
  failure: SessionFailure | undefined,
): 'idle' | 'shutdown' | 'killed' | 'crashed' {
  if (failure) return 'crashed';
  if (stopReason === 'shutdown') return 'shutdown';
  if (stopReason?.includes('idle')) return 'idle';
  return stopReason ? 'killed' : 'idle';
}

/** Remove a seeded DREAM.md only while it still matches the canonical source. */
function cleanupSeededDream(agentGroup: AgentGroup): void {
  try {
    ...
    if (fs.readFileSync(perGroupDream).equals(fs.readFileSync(sharedDream.hostPath))) fs.unlinkSync(perGroupDream);
  } catch (err) {
    log.warn('Failed to clean up seeded DREAM.md', { agentGroupId: agentGroup.id, err });
  }
}
...
export async function resolveProviderContribution(
  ...
  const context = {
    sessionDir: sessionDir(agentGroup.id, session.id),
    agentGroupId: agentGroup.id,
    groupDir: resolveGroupDir(agentGroup),
    selectedSkills: selectedSkillNames(containerConfig, agentGroup),
    hostEnv: process.env,
  };
...
export async function buildMounts(
  ...
  const containerSourceDir = path.resolve(
    process.env.NANOCLAW_CONTAINER_SOURCE_DIR || paths.containerSourceDir || path.join(projectRoot, 'container'),
  );
  ...
  const resolvedGroupDir = resolveGroupDir(agentGroup);
  const groupDir = fs.existsSync(resolvedGroupDir) ? fs.realpathSync(resolvedGroupDir) : path.resolve(resolvedGroupDir);
  ...
  if (contract) {
    providerSurfaces ??= await realizeProviderSpawnSurfaces(
      ...
      groupDir,
      sessDir,
      selectedSkillNames(containerConfig, agentGroup),
      {
        skillTargets: skillTargets(claudeDir, containerConfig, agentGroup, containerSourceDir),
        legacyOverlay: async () => providerContribution,
        composeProjectDocument: (spec) =>
          composeGroupProjectDoc(agentGroup, groupDir, spec, selectedSkillNames(containerConfig, agentGroup)),
      },
    );
    skillBackingPaths = providerSurfaces.skillBackingPaths;
  } else if (defaultSurfaces) {
    syncSkillSymlinks(claudeDir, containerConfig, agentGroup, containerSourceDir);

    // Compose CLAUDE.md fresh every spawn: every instruction source inlined
    // into one flat file. See `project-doc-compose.ts`.
    await composeGroupProjectDoc(
      agentGroup,
      groupDir,
      DEFAULT_PROJECT_DOC,
      selectedSkillNames(containerConfig, agentGroup),
  ...
  if (fs.existsSync(skillsSrc)) {
    mounts.push({
      hostPath: fs.realpathSync(skillsSrc),
  ...
  if (sharedDream && fs.existsSync(sharedDream.hostPath)) {
    try {
      ...
      log.warn('Failed to seed per-group DREAM.md before mount', { groupDir, err });
    }
    mounts.push({
      hostPath: fs.realpathSync(sharedDream.hostPath),
  ...
  if (fs.existsSync(globalDir)) {
    mounts.push({
      hostPath: fs.realpathSync(globalDir),
      containerPath: '/workspace/global',
      readonly: true,
      mountClass: 'shared-group-state',
      scope,
    });
  }
  ...
  // Workspace-public group state is an explicit read-only class admitted only
  // against the exact roots carried on SessionSpec.sharedStateRoots.
  const sharedGroups = resolveSharedGroups(agentGroup);
  if (sharedGroups.length > 0) {
    const sessionSharedRoot = path.join(sessDir, 'shared-groups');
    fs.mkdirSync(sessionSharedRoot, { recursive: true });
    for (const shared of sharedGroups) fs.mkdirSync(path.join(sessionSharedRoot, shared.folder), { recursive: true });
  }
  for (const shared of sharedGroups) {
    const unresolved = resolveGroupDir({ id: shared.id ?? '', folder: shared.folder });
    const sharedDir = fs.existsSync(unresolved) ? fs.realpathSync(unresolved) : path.resolve(unresolved);
    for (const [name, target] of [
      ['memory', 'memory'],
      ['IDENTITY.md', 'IDENTITY.md'],
      ['CLAUDE.md', 'CLAUDE.md'],
    ] as const) {
      const hostPath = path.join(sharedDir, name);
      if (!fs.existsSync(hostPath)) continue;
      mounts.push({
        hostPath,
        containerPath: `/workspace/shared-groups/${shared.folder}/${target}`,
        readonly: true,
        mountClass: 'shared-group-state',
        scope,
      });
  ...
  return mergeVolumeMounts([], mounts);
}

/** Last contributor wins one source per target, matching Docker's old argv ordering. */
export function mergeVolumeMounts(composed: VolumeMount[], contributed: VolumeMount[]): VolumeMount[] {
  return lastMountPerTarget([...composed, ...contributed]);
}
...
export interface ComposeSessionSpecInput {
   ...
   */
  /** Non-secret contribution from the embedded host's credential seam. */
...
function lastMountPerTarget<T extends { containerPath: string }>(mounts: T[]): T[] {
  const lastIndex = new Map<string, number>();
  mounts.forEach((mount, index) => lastIndex.set(mount.containerPath, index));
  return mounts.filter((mount, index) => lastIndex.get(mount.containerPath) === index);
}
...
export function composeSessionSpec(input: ComposeSessionSpecInput): SessionSpec {
  ...
  const env: Record<string, string> = {
    ...
    no_proxy: 'host.docker.internal,localhost,127.0.0.1',
    ...(sharedGroups.length > 0 ? { NANOCLAW_SHARED_GROUPS_JSON: JSON.stringify(sharedGroups) } : {}),
  };
  ...
  const agent: ContainerSpec = {
    ...
    env,
    // The image wrapper starts Camoufox and any future sidecars, then execs the
    // Bun runner; Docker's standard posture still supplies tini as PID 1.
    command: ['bash', '-c'],
    args: ['exec /app/start-with-camofox.sh'],
  ...
  const resolvedGroupHostPath = resolveGroupDir(agentGroup);
  const groupHostPath = fs.existsSync(resolvedGroupHostPath)
    ? fs.realpathSync(resolvedGroupHostPath)
    : path.resolve(resolvedGroupHostPath);
  const sharedStateRoots = mounts
    .filter((mount) => mount.mountClass === 'shared-group-state')
    .map((mount) => path.resolve(mount.hostPath));
...
export function syncSkillSymlinks(
  ...
  containerSourceDir = resolvedContainerSourceDir(),
): string[] {
  const skillsDir = path.join(claudeDir, 'skills');
  fs.mkdirSync(skillsDir, { recursive: true });
  const targets = skillTargets(claudeDir, containerConfig, group, containerSourceDir);
  const selected = selectGatewayAgentSkills([...targets.keys()]);
  for (const name of targets.keys()) if (!selected.includes(name)) targets.delete(name);
  for (const name of selected) if (!targets.has(name)) targets.set(name, `/app/skills/${name}`);

  for (const entry of fs.readdirSync(skillsDir)) {
    const entryPath = path.join(skillsDir, entry);
    try {
      if (fs.lstatSync(entryPath).isSymbolicLink() && !targets.has(entry)) fs.unlinkSync(entryPath);
    } catch {
      /* unreadable/missing */
    }
  }
...
function resolvedContainerSourceDir(): string {
  const paths = getEnginePaths();
  return path.resolve(process.env.NANOCLAW_CONTAINER_SOURCE_DIR || paths.containerSourceDir);
}
...
function skillTargets(
  ...
  const builtins = path.join(containerSourceDir, 'skills');
  if (fs.existsSync(builtins)) {
    for (const name of fs.readdirSync(builtins)) {
      try {
        if (fs.statSync(path.join(builtins, name)).isDirectory()) available.set(name, `/app/skills/${name}`);
      } catch {
        /* unreadable */
      }
    }
  ...
  for (const root of getExtraSkillRoots()) {
    if (!fs.existsSync(root.hostPath)) continue;
    for (const name of fs.readdirSync(root.hostPath)) {
      if (available.has(name) || (root.skillFilter && !root.skillFilter(name, group))) continue;
      try {
        if (fs.statSync(path.join(root.hostPath, name)).isDirectory()) {
          available.set(name, `${root.containerPath}/${name}`);
        }
      } catch {
        /* unreadable */
      }
    ...
    for (const name of fs.readdirSync(root)) {
      ...
      try {
        if (fs.statSync(path.join(root, name)).isDirectory()) {
          available.set(name, `/home/node/.claude/${subdir}/${name}`);
        }
      } catch {
        /* unreadable */
      }
  ...
  if (containerConfig.skills === 'all') return available;
  return new Map(containerConfig.skills.map((name) => [name, available.get(name) ?? `/app/skills/${name}`]));
}

/** Resolve selected skill names against the embedded host's source roots. */
function selectedSkillNames(
  containerConfig: import('./container-config.js').ContainerConfig,
  group: AgentGroup | null = null,
): string[] {
  const claudeDir = group ? path.join(DATA_DIR, 'v2-sessions', group.id, '.claude-shared') : '';
  return selectGatewayAgentSkills([
    ...skillTargets(claudeDir, containerConfig, group, resolvedContainerSourceDir()).keys(),
  ]);
}

...
it('keeps a Claude task billing failure in its task log and out of chat', async () => {
  ...
  await processQuery(
    query,
    { platformId: null, channelType: null, threadId: 'system:tasks:billing', inReplyTo: 't1' },
    ['t1'],
    'claude',
    null,
    false,
    'TestBot',
    [{ seriesId: 'billing', taskId: 't1', dispatched: [], assistantText: null, written: false }],
    null,
    undefined,
    'scheduled work',
    undefined,
  ...
  expect(readLatestTaskFire('billing')).toMatchObject({ status: 'error', errorMessage: BILLING_ERROR });
  expect(pushes).toHaveLength(0);
});
